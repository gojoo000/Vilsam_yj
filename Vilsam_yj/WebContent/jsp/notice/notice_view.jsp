...
<body>
	...
	<section id="articleForm">
		...
		<section id="basicInfoArea">
			...
			<a href="file_down?downFile=<%=article.getNOTICE_FILE()%>"> <%=article.getNOTICE_FILE() %>
			</a>
			<%} %>
		</section>
	...
	<section id="commandList">
		<a
			href="noticeReplyForm.no?notice_num=<%=article.getNOTICE_NUM() %>&page=<%=nowPage%>">
			[답변] </a> <a
			href="noticeModifyForm.no?notice_num=<%=article.getNOTICE_NUM() %>">
			[수정] </a> <a
			href="noticeDeleteForm.no?notice_num=<%=article.getNOTICE_NUM() %>&page=<%=nowPage%>">
